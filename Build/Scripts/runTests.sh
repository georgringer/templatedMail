...
read -r -d '' HELP <<EOF
templatedmail test runner. Execute unit test suite and some other details.
...
Options:
    ...
    -e "<phpunit or codeception options>"
        ...
        Additional options to send to phpunit (unit & functional tests) or codeception (tests).
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/templatedmail/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/templatedmail/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/templatedmail/Tests/Unit"
            ;;
    esac
fi
